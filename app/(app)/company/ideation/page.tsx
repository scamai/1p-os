...
function Section({
  ...
  return (
    <div className="rounded-xl border border-black/[0.08] bg-white p-5">
      <label className="block text-[13px] font-semibold text-black/70">
        {label}
      </label>
      <p className="mt-0.5 text-[12px] text-black/40 mb-3">{hint}</p>
      <textarea
        value={value}
        onChange={(e) => onChange(e.target.value)}
        rows={rows}
        className="w-full rounded-md border border-black/[0.08] bg-white px-3 py-2 text-[13px] text-black placeholder:text-black/40 focus:outline-none focus:ring-2 focus:ring-black resize-y leading-relaxed"
...
export default function IdeationPage() {
  ...
  return (
    <div className="mx-auto max-w-[800px]">
      ...
      <div>
        <h1 className="text-lg font-semibold text-black">Ideation</h1>
          ...
          {sections.map((s, i) => (
            <div
              ...
              className={`h-1.5 w-8 rounded-full transition-colors ${
                s.filled ? "bg-black" : "bg-black/[0.08]"
              }`}
            />
          ))}
        </div>
        ...
        <div className="rounded-xl border border-black/[0.08] bg-white p-5">
          <div className="flex items-center justify-between mb-1">
            ...
            <button
              ...
              className="h-7 rounded-md bg-black px-2.5 text-[12px] font-medium text-white transition-opacity hover:opacity-90"
            >
          ...
          {showCompForm && (
            <form
              ...
              className="mb-4 rounded-lg border border-black/[0.08] bg-black/[0.02] p-4 space-y-3"
            >
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="h-8 w-full rounded-md border border-black/[0.08] bg-white px-3 text-[13px] text-black placeholder:text-black/40 focus:outline-none focus:ring-2 focus:ring-black"
              ...
              <div className="flex justify-end">
                <button
                  type="submit"
                  className="h-7 rounded-md bg-black px-3 text-[12px] font-medium text-white transition-opacity hover:opacity-90"
                >
          ...
          {data.competitors.length > 0 ? (
            <div className="space-y-2">
              {data.competitors.map((c) => (
                <div
                  key={c.id}
                  className="group rounded-lg border border-black/[0.04] bg-black/[0.02] px-4 py-3 hover:bg-white transition-colors"
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="hidden group-hover:flex items-center gap-1 shrink-0 ml-2">
                      <button
                        ...
                        className="rounded px-2 py-1 text-[11px] text-black/50 hover:text-black hover:bg-black/[0.04] transition-colors"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => deleteComp(c.id)}
                        className="rounded px-2 py-1 text-[11px] text-black/40 hover:text-black hover:bg-black/[0.04] transition-colors"
                      >
                        Delete
                      </button>
                    </div>
