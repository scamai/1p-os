      ...
      <div className="border border-black/[0.08] bg-white p-5 mb-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-0 border border-black/[0.08]">
          {(["tam", "sam", "som"] as const).map((key, i) => {
            ...
            const m = data[key];
            return (
              <div key={key} className={`p-4 ${i < 2 ? "border-r border-black/[0.08]" : ""}`}>
                <p className="text-[11px] font-bold text-black/50 uppercase tracking-widest mb-1">{labels[key]}</p>
                <p className="text-[10px] text-black/40 mb-3">{descriptions[key]}</p>
                <div className="flex items-center gap-1 mb-2">
                  ...
                  <input type="number" min={0} value={m.value}
                    ...
                    className="w-20 text-[13px] border border-black/[0.08] px-2 py-1 text-black text-center focus:outline-none focus:border-black/40" />
                  <select value={m.unit}
                    onChange={(e) => setData((prev) => ({ ...prev, [key]: { ...prev[key], unit: e.target.value } }))}
                    className="text-[12px] border border-black/[0.08] px-1 py-1 text-black/70 focus:outline-none focus:border-black/40">
                    <option value="K">K</option><option value="M">M</option><option value="B">B</option><option value="T">T</option>
                  </select>
                </div>
                <p className="text-xl font-bold font-mono text-black">{fmt(m.value, m.unit)}</p>
                <input type="text" value={m.source} placeholder="Source..."
                  onChange={(e) => setData((prev) => ({ ...prev, [key]: { ...prev[key], source: e.target.value } }))}
                  className="mt-2 w-full text-[11px] border border-black/[0.04] px-2 py-1 text-black/50 placeholder:text-black/30 focus:outline-none focus:border-black/40" />
              </div>
            );
          })}
        </div>
        <div className="mt-3 flex items-center justify-center gap-3 text-[12px] text-black/40">
          <span className="font-mono font-medium text-black/70">{fmt(data.tam.value, data.tam.unit)}</span>
          <span>-&gt;</span>
          <span className="font-mono font-medium text-black/70">{fmt(data.sam.value, data.sam.unit)}</span>
          <span>-&gt;</span>
          <span className="font-mono font-medium text-black/70">{fmt(data.som.value, data.som.unit)}</span>
        </div>
      </div>
      ...
      <div className="border border-black/[0.08] bg-white p-5 mb-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={() => setEditingComp({ id: uid(), name: "", url: "", pricing: "", strengths: "", weaknesses: "" })}
            className="text-[12px] px-3 py-1 bg-black text-white hover:bg-black/80">Add Competitor</button>
        </div>
            ...
            <div className="bg-white p-4 sm:p-6 w-full mx-2 sm:mx-0 sm:max-w-md shadow-xl border border-black/[0.08]">
              ...
              <div className="space-y-3">
                {([["name", "Name"], ["url", "Website"], ["pricing", "Pricing"]] as const).map(([f, l]) => (
                  <div key={f}>
                    <label className="block text-[11px] text-black/50 mb-1">{l}</label>
                    <input type="text" value={editingComp[f]} onChange={(e) => setEditingComp({ ...editingComp, [f]: e.target.value })}
                      className="w-full text-[13px] border border-black/[0.08] px-2 py-1.5 text-black focus:outline-none focus:border-black/40" />
                  </div>
                ))}
                {([["strengths", "What they do well"], ["weaknesses", "Where they fall short"]] as const).map(([f, l]) => (
                  <div key={f}>
                    ...
                    <textarea value={editingComp[f]} onChange={(e) => setEditingComp({ ...editingComp, [f]: e.target.value })} rows={2}
                      className="w-full text-[13px] border border-black/[0.08] px-2 py-1.5 text-black focus:outline-none focus:border-black/40 resize-none" />
                  </div>
                ))}
              </div>
              <div className="flex justify-end gap-2 mt-4">
                <button onClick={() => setEditingComp(null)} className="text-[12px] px-3 py-1.5 border border-black/[0.08] text-black/60 hover:bg-black/[0.02]">Cancel</button>
                <button onClick={saveComp} className="text-[12px] px-3 py-1.5 bg-black text-white hover:bg-black/80">Save</button>
              </div>
            </div>
          </div>
        )}
        ...
        {data.competitors.length === 0 ? (
          <p className="text-[13px] text-black/30 text-center py-6">No competitors tracked yet. Every market has competitors — if you think yours doesn&apos;t, look harder.</p>
        ) : (
