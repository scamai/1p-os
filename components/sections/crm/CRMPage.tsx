        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <p className="text-[13px] font-medium text-black">{contact.name}</p>
              <span className="rounded bg-black/[0.04] px-1.5 py-0.5 text-[10px] text-black/50">
                {contact.type}
              </span>
              {contact.status !== "Active" && (
                <span className="text-[10px] text-black/40">{contact.status}</span>
              )}
            </div>
      ...
      {expanded && (
        <div className="border-b border-black/[0.04] bg-black/[0.02] px-4 py-3">
          <div className="grid grid-cols-2 gap-4 text-[12px]">
            <div>
              <p className="text-[11px] text-black/40">Email</p>
              <p className="mt-0.5 text-black/60">{contact.email}</p>
            </div>
            <div>
              <p className="text-[11px] text-black/40">Last Agent</p>
              <p className="mt-0.5 text-black/60">{contact.lastAgent}</p>
            </div>
          </div>
          <div className="mt-3">
            <p className="text-[11px] text-black/40">Notes</p>
            <p className="mt-0.5 text-[12px] text-black/60">{contact.notes}</p>
          </div>
        </div>
      )}
    </>
  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="flex items-center justify-between">
        <h1 className="text-lg font-semibold text-black">CRM</h1>
        <button
      ...
      <div className="mt-4 grid grid-cols-3 gap-6">
        <div>
          <p className="text-[11px] text-black/50">Clients</p>
        ...
        <div>
          <p className="text-[11px] text-black/50">Total Revenue</p>
          <p className="mt-1 font-mono text-lg font-semibold text-black">
      ...
      <div className="mt-6">
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="h-14 animate-pulse rounded bg-black/[0.04]" />
            ))}
          </div>
        ) : (
          <>
