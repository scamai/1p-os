  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-black">Channels</h1>
      ...
      {configError && (
        <div className="mt-4 rounded-lg border border-black/[0.08] bg-black/[0.04] px-4 py-3">
          <div className="flex items-start gap-2">
            <svg className="h-4 w-4 shrink-0 text-black/50 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
            <div className="flex-1">
              <p className="text-sm font-medium text-black/80">{configError.message}</p>
              <p className="mt-1 text-xs text-black/60">
                Add the required environment variables to your <code className="rounded bg-black/[0.08] px-1">.env.local</code> file, then restart the server.
              </p>
            </div>
            <button onClick={() => setConfigError(null)} className="text-black/40 hover:text-black/60">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-black/50">
                        {isConnected ? (
                          <>
                            <span className="inline-block h-1.5 w-1.5 rounded-full bg-black mr-1" />
                            {integration.label}
                          ...
                          <span className="text-black">Connection error — reconnect</span>
                        ) : (
                          <span className="text-black/40">
      ...
      <div className="mt-10">
        ...
        {!composioEnabled ? (
          <div className="mt-3 rounded-lg border border-dashed border-black/30 bg-black/[0.02] p-4">
            <p className="text-sm text-black/70">
              Connect 500+ apps — GitHub, Jira, Salesforce, HubSpot, Notion, Linear, and more.
            </p>
            <p className="mt-1 text-xs text-black/50">
              Add your <code className="rounded bg-black/[0.08] px-1">COMPOSIO_API_KEY</code> to{" "}
              <code className="rounded bg-black/[0.08] px-1">.env.local</code> to enable.
              Get a key at{" "}
              <a
            ...
            {showComposioApps && (
              <div className="mt-3">
                  ...
                  {COMPOSIO_POPULAR_APPS.filter((app) =>
                    ...
                    return (
                      ...
                      >
                        ...
                        {isConnected && (
                          <svg className="ml-auto h-3 w-3 text-black" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                            <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                          </svg>
                        )}
                        {composioConnecting === app.slug && (
                          <span className="ml-auto text-[10px] text-black/40">...</span>
                        )}
                      </button>
                    );
                  })}
                ...
                <p className="mt-2 text-[10px] text-black/40">
                  Showing popular apps. Your agents can discover all 500+ tools automatically via <code className="bg-black/[0.04] rounded px-0.5">composio_list_tools</code>.
                </p>
              </div>
            )}
          </>
      ...
      {integrations.length > 0 && (
        <div className="mt-10">
          ...
          <div className="mt-3 rounded-lg border border-black/[0.08] overflow-hidden">
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-black/[0.08] bg-black/[0.02]">
                  <th className="px-3 py-2 text-left font-medium text-black/50">Service</th>
                  <th className="px-3 py-2 text-left font-medium text-black/50">Account</th>
                  <th className="px-3 py-2 text-left font-medium text-black/50">Status</th>
                  <th className="px-3 py-2 text-right font-medium text-black/50">Connected</th>
                </tr>
              </thead>
              <tbody>
                {integrations.map((integration) => {
                  ...
                  return (
                    <tr key={integration.id} className="border-b border-black/[0.04] last:border-0">
      ...
      {/* What agents can do */}
      <div className="mt-10 rounded-lg border border-black/[0.08] bg-black/[0.02] p-4">
        <h3 className="text-xs font-semibold text-black/70 uppercase tracking-wider">
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          {apiKeyError && (
            <p className="text-xs text-black/80">{apiKeyError}</p>
          )}

          <div className="mt-2 flex items-center justify-end gap-2">
            <button
              onClick={() => { setApiKeyModal(null); setApiKeyError(null); }}
              className="rounded-md px-3 py-1.5 text-xs text-black/60 hover:text-black transition-colors"
            >
              Cancel
            </button>
            <Button
      ...
      >
        <div className="flex flex-col gap-3">
          {confirmDisconnect && (
            <div className="rounded-md bg-black/[0.02] border border-black/[0.08] p-3 text-xs text-black/60">
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              className="rounded-md bg-black px-3 py-1.5 text-xs text-white hover:bg-black/80 transition-colors"
            >
              Disconnect
            </button>
          </div>
      ...
      >
        <div className="flex flex-col gap-3">
          {logContent ? (
            <pre className="max-h-64 overflow-auto rounded-md bg-black/[0.02] border border-black/[0.08] p-3 text-[11px] text-black/70 whitespace-pre-wrap">
              {logContent}
            </pre>
          ) : (
            <p className="text-xs text-black/50">No logs for today yet.</p>
          )}

          {logFiles.length > 0 && (
            <div className="text-xs text-black/50">
              <p className="font-medium text-black/70 mb-1">Available log files:</p>
              <div className="flex flex-wrap gap-1">
                {logFiles.slice(0, 10).map((f) => (
                  <span key={f} className="rounded bg-black/[0.04] px-1.5 py-0.5 text-[10px]">
                    {f.replace(".md", "")}
                  </span>
                ))}
              </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-md bg-black px-3 py-1.5 text-xs text-white hover:bg-black/80 transition-colors"
            >
...
function CheckIcon() {
  return (
    <svg className="h-3 w-3 text-black shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
    </svg>
  );
}
