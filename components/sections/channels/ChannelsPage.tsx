  ...
  return (
    <div className="mx-auto max-w-3xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-semibold text-slate-900">Channels</h1>
      ...
      {configError && (
        <div className="mt-4 rounded-lg border border-slate-200 bg-slate-100 px-4 py-3">
          <div className="flex items-start gap-2">
            <svg className="h-4 w-4 shrink-0 text-slate-500 mt-0.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
              <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z" />
            </svg>
            <div className="flex-1">
              <p className="text-sm font-medium text-slate-800">{configError.message}</p>
              <p className="mt-1 text-xs text-slate-600">
                Add the required environment variables to your <code className="rounded bg-slate-200 px-1">.env.local</code> file, then restart the server.
              </p>
            </div>
            <button onClick={() => setConfigError(null)} className="text-slate-400 hover:text-slate-600">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-[11px] text-slate-500">
                        {isConnected ? (
                          <>
                            <span className="inline-block h-1.5 w-1.5 rounded-full bg-slate-900 mr-1" />
                            {integration.label}
                          ...
                          <span className="text-slate-900">Connection error — reconnect</span>
                        ) : (
                          <span className="text-slate-400">
        ...
        {!composioEnabled ? (
          <div className="mt-3 rounded-lg border border-dashed border-slate-300 bg-slate-50 p-4">
            ...
            <p className="mt-1 text-xs text-slate-500">
              Add your <code className="rounded bg-slate-200 px-1">COMPOSIO_API_KEY</code> to{" "}
              <code className="rounded bg-slate-200 px-1">.env.local</code> to enable.
              Get a key at{" "}
              <a
            ...
            {showComposioApps && (
              <div className="mt-3">
                  ...
                  {COMPOSIO_POPULAR_APPS.filter((app) =>
                    ...
                    return (
                      ...
                      >
                        ...
                        {isConnected && (
                          <svg className="ml-auto h-3 w-3 text-slate-900" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                ...
                <p className="mt-2 text-[10px] text-slate-400">
                  Showing popular apps. Your agents can discover all 500+ tools automatically via <code className="bg-slate-100 rounded px-0.5">composio_list_tools</code>.
                </p>
              </div>
            )}
          </>
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-slate-200 bg-slate-50">
                  <th className="px-3 py-2 text-left font-medium text-slate-500">Service</th>
                  <th className="px-3 py-2 text-left font-medium text-slate-500">Account</th>
                  <th className="px-3 py-2 text-left font-medium text-slate-500">Status</th>
                  <th className="px-3 py-2 text-right font-medium text-slate-500">Connected</th>
                </tr>
              </thead>
              <tbody>
                {integrations.map((integration) => {
                  ...
                  return (
                    <tr key={integration.id} className="border-b border-slate-100 last:border-0">
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          {apiKeyError && (
            <p className="text-xs text-slate-800">{apiKeyError}</p>
          )}

          <div className="mt-2 flex items-center justify-end gap-2">
            <button
              onClick={() => { setApiKeyModal(null); setApiKeyError(null); }}
              className="rounded-md px-3 py-1.5 text-xs text-slate-600 hover:text-slate-900 transition-colors"
            >
              Cancel
            </button>
            <Button
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <button
              ...
              className="rounded-md bg-slate-900 px-3 py-1.5 text-xs text-white hover:bg-slate-800 transition-colors"
            >
              Disconnect
            </button>
          </div>
      ...
      >
        <div className="flex flex-col gap-3">
          {logContent ? (
            <pre className="max-h-64 overflow-auto rounded-md bg-slate-50 border border-slate-200 p-3 text-[11px] text-slate-700 whitespace-pre-wrap">
              {logContent}
            </pre>
          ) : (
            <p className="text-xs text-slate-500">No logs for today yet.</p>
          )}

          {logFiles.length > 0 && (
            <div className="text-xs text-slate-500">
              <p className="font-medium text-slate-700 mb-1">Available log files:</p>
              <div className="flex flex-wrap gap-1">
                {logFiles.slice(0, 10).map((f) => (
                  <span key={f} className="rounded bg-slate-100 px-1.5 py-0.5 text-[10px]">
                    {f.replace(".md", "")}
                  </span>
                ))}
              </div>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="flex items-center gap-1 rounded-md bg-slate-900 px-3 py-1.5 text-xs text-white hover:bg-slate-800 transition-colors"
            >
...
function CheckIcon() {
  return (
    <svg className="h-3 w-3 text-slate-900 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
    </svg>
  );
}
