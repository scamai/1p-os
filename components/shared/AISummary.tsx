...
function AISummary({ section, className = "" }: AISummaryProps) {
  ...
  if (loading) {
    return (
      <div className={`flex items-center gap-2 ${className}`}>
        <div className="h-[2px] w-full max-w-[200px] overflow-hidden rounded-full bg-black/[0.04]">
          <div className="h-full w-1/2 animate-pulse rounded-full bg-black/60" style={{
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`}>
      <SparkleIcon className="h-3 w-3 flex-shrink-0 text-black" />
      <p className="text-[13px] italic text-black/50">{summary}</p>
    </div>
  );
}
